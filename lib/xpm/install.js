...
class Install extends CliCommand {
  ...
  async installAllDependencies () {
    ...
    if (config.configurationName) {
      ...
      await this.purgeNodeModules()
    }
  }

  async purgeNodeModules () {
    const log = this.log
    ...
    const nodeModulesPath = path.join(config.cwd,
      context.globalConfig.localNpmFolderName)
    const binPath = path.join(nodeModulesPath,
      context.globalConfig.dotBin)

    try {
      // If the path is not present, throw.
      log.trace(`stat ${binPath}`)
      await fsPromises.stat(binPath)
    } catch (err) {
      log.trace(`del ${nodeModulesPath}`)
      await del(nodeModulesPath, { force: true })

      return
    }

    // Enumerate packages and remove xPacks.
    await this.purgeNodeModulesRecursive(nodeModulesPath, 2)
  }

  async purgeNodeModulesRecursive (folderPath, depth) {
    const log = this.log
    log.verbose(folderPath)

    const xpack = this.xpack

    const json = await xpack.isFolderPackage(folderPath)
    if (xpack.isXpack(json)) {
      log.trace(`del ${folderPath}`)
    ...
    let dirents = await fsPromises.readdir(folderPath,
      { withFileTypes: true })

    for (const dirent of dirents) {
      if (dirent.isDirectory()) {
        const subFolderPath = path.join(folderPath, dirent.name)
        await this.purgeNodeModulesRecursive(subFolderPath, depth - 1)
      }
    }

    dirents = await fsPromises.readdir(folderPath,
      { withFileTypes: true })
  ...
  async downloadAndLinkOneDependency ({
    ...
    // First try to find dependency in the system package path.
    let installedPackagePath = path.join(
      context.globalConfig.systemFolderPath,
      manifestIds.getPath())

    let installedJson = await xpack.isFolderPackage(installedPackagePath)

    if (!installedJson) {
      // Not on system package path so try global package path.
      installedPackagePath = path.join(
        context.globalConfig.globalFolderPath,
        manifestIds.getPath())

      installedJson = await xpack.isFolderPackage(installedPackagePath)
    }

    if (!installedJson) {
      // The package does not exist in the system or global repos.
      if (config.isDryRun) {
        verboseMessage = `Pretend adding '${manifestIds.getFullName()}' to ` +
          `repository as '${installedPackagePath}'...`
      } else {
      ...
      await this.pacoteExtractContentReadOnly({
        packFullName: manifestIds.getFullName(),
        fromFolderPath: manifestIds.getPacoteFrom(),
        destinationFolderPath: installedPackagePath,
        cacheFolderPath,
        verboseMessage
      })
    ...
    assert(installedJson)

    if (!installedJson.xpack) {
      if (log.isVerbose()) {
    ...
    if (installedJson.xpack || manifest.isContributedByDevDependencies) {
      await this.addFolderLinkToGlobalRepo({
        globalJson: installedJson,
        manifestIds,
        globalPackagePath: installedPackagePath,
        buildFolderRelativePath
      })

      try {
        // xPack
        if (xpack.isBinaryXpack(installedJson)) {
          await this.addBinLinks({
            bins: installedJson.xpack.bin,
            fromFolderPath: installedPackagePath,
            localFolderName: manifestIds.getFolderName(),
            globalFolderRelativePath: manifestIds.getPath(),
            destFolderPath: xpacksPath,
            buildFolderRelativePath
  ...
  async addFolderLinkToGlobalRepo ({
    ...
    if (os.platform() === 'win32') {
      ...
      try {
        ...
        // console.log(absPath)
        log.trace(err)
        // Just silently fail to create links on non-NTFS or remote resources
  ...
  async pacoteExtractContentReadOnly ({
    ...
    if (!json || !json.xpack) {
      if (!this.policies.shareNpmDependencies) {
        log.trace(`del(${destinationFolderPath})`)
        await del(destinationFolderPath, { force: true })
        throw new CliError(
